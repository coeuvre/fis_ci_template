...
class CI_Template {
   ...
   var $regions = array(
      '_scripts' => array(
          'content' => array(self::SCRIPT_PLACEHOLDER)
      ),
      '_styles' => array(
          'content' => array(self::STYLE_PLACEHOLDER)
      ),
   );
   var $output;
   var $js = array();
   ...
   // --------------------------------------------------------------------
   function get_namespace(&$id, $def_ns = null)
   {
      $pos = strpos($id, ':');
      if ($pos === false)
      {
   ...
   function get_info(&$id, $caller_ns = null, &$ns = null, &$map = null)
   {
      $ns = $this->get_namespace($id, $caller_ns);
      if (isset($this->maps[$ns]))
      {
         $map = $this->maps[$ns];
      }
      else
      {
         if (isset($this->config['map_dir']))
         {
            $map_dir = $this->config['map_dir'];
            if ($ns === '__global__')
            {
            ...
            {
               show_error('unable to load reource map [' . $map_file . ']', E_USER_ERROR);
            }
         }
         else
         {
            show_error('undefined resource map dir', E_USER_ERROR);
         }
      }
      ...
      {
          show_error('undefined resource [' . $id . ']', E_USER_ERROR);
      }
      return null;
   }
   ...
   function import($id, $caller_ns = null)
   {
      ...
      {
         $info = $this->get_info($id, $caller_ns, $ns, $map);
         if($info)
         {
            $uri = $info['uri'];
            $type = $info['type'];
            if (isset($info['pkg']))
            {
               ...
               {
                  $this->imported[$rId] = $uri;
               }
            }
            else
            {
               $this->imported[$id] = $uri;
            }
   ...
   function render($region = NULL, $buffer = FALSE, $parse = FALSE)
   {
      ...
      else
      {
         ...
         else
         {
            ...
            //
            // NOTICE: the third argument is `true`, so CI will always return the
            //         rendered view as string rather than sending it to browser
            $output = $this->CI->load->view($this->master, $this->output, TRUE);
         }

         $output = str_replace(self::STYLE_PLACEHOLDER, implode($this->css), $output);
         $output = str_replace(self::SCRIPT_PLACEHOLDER, implode($this->js), $output);
